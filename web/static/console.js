...
/** Add the selected server code to a slug generated by the web UI. */
...
/** Display either accepted datetime form with a space between date and time. */
function displayStamp(text) {
  const value = readStamp(text);
  return value ? writeStamp(value) : String(text || '');
}
